...
public class MqttRemote {
    ...
    private void connect() {
        ...
        mqttClient.setCallback(new MqttCallbackExtended() {
            ...
            public void connectComplete(boolean reconnect, String serverURI) {
                ...
                try {
                    ...
                    mqttClient.publish(getMqttSubTopic("status"), status.toString().getBytes(), 0, true);
        ...
        connectOptions.setWill(getMqttSubTopic("status"), status.toString().getBytes(), 0, true);
    ...
    private String getMqttSubTopic(@NonNull String subTopic) {
        return "iot-2/evt/" + subTopic + "/fmt/json";
    }
    ...
    private void onMqttMessage(@NonNull String topic, @NonNull MqttMessage message) {
        if (getMqttSubTopic("shutter").equals(topic)) {
            ...
            camera.refocus();
        } else if (getMqttSubTopic("auto_focus").equals(topic)) {
            camera.setAutoFocus("1".equals(new String(message.getPayload())));
        } else if (getMqttSubTopic("dim_screen").equals(topic)) {
            camera.setDimScreen("1".equals(new String(message.getPayload())));
        } else if (getMqttSubTopic("disable_preview").equals(topic)) {
